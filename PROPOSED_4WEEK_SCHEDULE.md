...
> proposals. New content (HW2 rebuild, stylometry exercise) is drafted separately
> in the repo.
...
| 5 | Functions & dictionaries + recap | CA Dictionaries/Functions | — → **assign HW2 (rebuilt, Walsh-independent)** |

### Week 2 — From text to data: ethics, pandas, AI
| Day | Focus | F25 assets | Critical frame |
|----|-------|-----------|----------------|
| 6 | HW2 debrief + "Meaningful Words" term frequency | HW1 logic | data as evidence → **assign HW1** |
...
| 10 | Workshop: collect & clean **your** cultural dataset | (TenCommandments CSVs as model) | — → sets up HW4-1 |
...
| 13 | HW4-1 work session (freq + sentiment on own data) | HW4-1 | predictions on the record |
| 14 | Topic modeling intro (Gensim LDA) | CA Topic Modeling | ML7 NLP & Topic Modeling |
| 15 | Topic modeling deep · num_topics · limits | CA Topic Modeling | — → **assign HW4-2** |

### Week 4 — Integration & data-driven opinions (capstone)
| Day | Focus | F25 assets | Notes |
|----|-------|-----------|-------|
| 16 | Integration: freq + sentiment + topics **+ short stylometry demo** | HW4-2 + stylometry notebook (Part A) | "measure what you close-read on Day 7" — reinforces close→distant→close |
| 17 | "Being wrong as learning" · validation · capstone framing | HW4-2 reflection | frames the **stylometry capstone option** |
...
## Anchor assignments (down from F25's HW1–HW8)
- **HW2** — comparisons/conditionals/lists/loops (rebuilt, Walsh-independent) — end of Wk1
- **HW1** — term frequency — Wk2
- **HW4-1** — term frequency + sentiment — Wk3
- **HW4-2** — topic modeling + integration — Wk4
...
2. **Topic-modeling install** — the HW4-2 pinned-deps + runtime-restart cell must be tested on Colab's 2026 image before Day 14.
